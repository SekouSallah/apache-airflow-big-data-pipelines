"""
Module : dev.sekousow.kafkastreams.airflow.dags.pipeline_big_data_parallele

Pipeline Big Data avec exécution parallèle de deux branches analytiques.
Illustre le pattern fan-out / fan-in :
  Préparation → Validation → [Ville, Produit] → Rapport final

Ce DAG démontre la capacité d'Airflow à orchestrer des dépendances
non linéaires dans un même workflow.
...
def preparation_donnees():
    """Génère le fichier de ventes nettoyé avec la colonne montant pré-calculée."""
...
def validation_donnees():
    """Vérifie la présence du fichier de données avant d'engager les traitements."""
...
def traitement_par_ville():
    ...
    Branche parallèle n°1 du pipeline.
    """
    resultats = {}
...
def traitement_par_produit():
    ...
    Branche parallèle n°2 du pipeline.
    """
    resultats = {}
...
def generation_rapport_final():
    """
    Fusionne les résultats des deux branches parallèles dans un rapport unique.
    Étape de synchronisation (fan-in) du pattern parallèle.
    """
    with open(RESULT_VILLE, mode="r", encoding="utf-8") as f:
        par_ville = json.load(f)

    with open(RESULT_PRODUIT, mode="r", encoding="utf-8") as f:
        par_produit = json.load(f)
